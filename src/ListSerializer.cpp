...
std::unordered_map<ListNode *, int> get_node_indexes_map(const std::vector<ListNode *> &nodes)
{
    std::unordered_map<ListNode *, int> indexes;

    for (size_t i = 0; i < nodes.size(); ++i)
    {
        indexes[nodes[i]] = static_cast<int>(i);
    }

    return indexes;
}

int get_node_index(const ListNode *node, const std::unordered_map<ListNode *, int> &indexes)
{
    if (node == nullptr)
        return -1;

    if (auto it = indexes.find(const_cast<ListNode *>(node)); it != indexes.end())
        return it->second;
...
void write_to_binary_file(const std::string &filename, const std::vector<ListNode *> &nodes)
{
    ...
    auto indexes_map = get_node_indexes_map(nodes);

    for (size_t i = 0; i < nodes.size(); ++i)
    {
        ListNode *node = nodes[i];

        int rand_index = get_node_index(node->rand, indexes_map);
